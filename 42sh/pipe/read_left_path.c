...
static char	*after_while(t_env **ev, t_env *en, t_edit **lst, t_hist **hst)
{
	char	*str;
...
static int	in_while(char **key, t_env **data, t_hist **hst, t_edit **lst)
{
	int		value;
...
char		*ft_read_htdoc(t_env **env, t_env *environ)
{
	char	*str;
	t_hist	*hst;
	t_edit	*lst_e;
...
int			ft_db_ar_lft(t_pipe **tmp, t_env *env, int flag)
{
	char	*str;
